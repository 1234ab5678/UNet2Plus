...
from nets.UNet2Plus import UNet2Plus
...
if __name__ == "__main__":
    ...
    #   数据集路径
    #------------------------------#
    VOCdevkit_path  = 'F:/Project/Table-Recognition/Dataset/Table-Structure-Segmentation/3Classes/Patch640/'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    model = UNet2Plus(num_classes=num_classes, backbone=backbone, pretrained=pretrained).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if True:
        ...
        train_dataset   = UnetDataset(train_lines, input_shape, num_classes, True, VOCdevkit_path, "train", True)
        val_dataset     = UnetDataset(val_lines, input_shape, num_classes, False, VOCdevkit_path, "val", False)
        ...
        gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = batch_size, num_workers = num_workers, pin_memory=True,
                                    drop_last = False, collate_fn = unet_dataset_collate, sampler=train_sampler)
        gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, num_workers = num_workers, pin_memory=True, 
                                    drop_last = False, collate_fn = unet_dataset_collate, sampler=val_sampler)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            flag=fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, 
                    epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, msssim_loss, iou_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir, local_rank)
            if flag:
                count=0
            else:
                count=count+1
